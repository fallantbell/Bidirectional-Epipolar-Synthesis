...
class MultiEmbedder(nn.Module):
    ...
    def process(self, batch):
        ...
        for k in ["R_rel", "t_rel", "K", "K_inv"]:
            # print(f"batch[{k}].shape = {batch[k].shape}")
            entry = batch[k].reshape(batch[k].shape[0], -1, self.n_channels)
            inputs.append(entry)
            
        x = torch.cat(inputs, dim=1)
        # print(f"x.shape = {x.shape}")
        # print(f"n_position = {self.n_positions}")
        assert x.shape[1] == self.n_positions, x.shape
        x = self.fc(x)
        return x

class MultiEmbedder_v2(nn.Module):
    def __init__(self, keys, n_positions, n_channels, n_embed, bias=False):
        super().__init__()
        self.keys = keys
        self.n_positions = n_positions
        ...
        self.fc_rel = nn.Linear(self.n_channels, self.n_embed, bias=bias)
        self.fc_abs = nn.Linear(self.n_channels, self.n_embed, bias=bias)
    ...
    def process_abs(self, batch):
        inputs = []
        for k in ["R", "t", "K", "K_inv"]:
            entry = batch[k].reshape(batch[k].shape[0], -1, self.n_channels)
            inputs.append(entry)
