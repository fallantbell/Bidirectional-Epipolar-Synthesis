...
def pose_to_sparse(txt_src, img_src, spa_dst, DEBUG=False, exists_ok=True,
    ...
    for txt in txts:
        ...
        filenames = [str(ts)+".jpg" for ts in timestamps]
        ...
        if not len(glob.glob(os.path.join(opt.img_src, vidid, "*.jpg"))) == len(filenames):
            failed.append(vidid)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_src', type=str,
                        help='path to directory containing <vidid>/<timestamp>.jpg frames')
